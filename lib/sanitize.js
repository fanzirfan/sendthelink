...
export const sanitizeInput = (input, allowBasicHTML = false) => {
    ...
    // NOTE: We used to try to strip tags with regex but it's prone to bypass.
    // Ideally we'd use DOMPurify but for Vercel Edge/performance reasons we'll rely on
    // strict HTML escaping which renders any injected scripts inert.
    let sanitized = input;

    // If you absolutely MUST strip basic tags (for plain text), use a library.
    // For now, we will trust escapeHtml to neutralize XSS.
